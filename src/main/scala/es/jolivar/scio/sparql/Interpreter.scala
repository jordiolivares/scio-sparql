...
object Interpreter {
  ...
  implicit class SCollectionStatements(val col: SCollection[Statement])
    ...
    /** Execute the query as is on the collection of triples provided by this SCollection
      * @param query
      * @return the results of the SPARQL query with potential duplicates
      */
    def executeSparql(
        query: String
    ): SCollection[BindingSet] = {
      executeSparql(query, EmptyBindingSet.getInstance())
    }

    /** Execute the query with the given bindings on the collection of triples provided by this SCollection.
      * The query used in practice will be generated by replacing all instances of the bindings within the query.
      *
      * For example, if we have the query `SELECT ?a WHERE { ?a ?b ?c }` and the binding `?c -> <iri>` then the query
      * used will be `SELECT ?a WHERE { ?a ?b <iri> }`.
      * @param query
      * @param bindings
      * @return the results of the SPARQL query with potential duplicates
      */
    def executeSparql(
      ...
      val generatedQuery = QueryStringUtil.getTupleQueryString(query, bindings)
      val parsedQuery =
  ...
  def processOperation(fullDataset: SCollection[Statement])(
    ...
    val results = fullDataset.transform(tupleExpr.getSignature) { fullDataset =>
      tupleExpr match {
        case order: Order =>
        ...
        case join: Join =>
          (join.getLeftArg, join.getRightArg) match {
            ...
            case _ =>
              ...
              keyedLeft.join(keyedRight).values.map { case (left, right) =>
                left ++ right
              }
          }
        case leftJoin: LeftJoin =>
          ...
          val keyedRight =
            rightDataset.keyBy(commonBindings.getBindingsForKeying)
          keyedLeft.leftOuterJoin(keyedRight).values.map { case (left, right) =>
        ...
        case group: Group =>
          ...
          val keyedResults =
            results.keyBy(bindingsToGroupBy.getBindingsForKeying)
              ...
              case sample: Sample =>
                ...
                val evaluatedExpr =
                  keyedResults
                    ...
                    .collect { case (key, resultSet -> literal) =>
                      key -> (resultSet -> literal)
                    }
                val samplePerKey = if (sample.isDistinct) {
                  evaluatedExpr
                    .map { case (key, pair @ (_, lit)) =>
                      (key, lit.toString) -> pair
                    }
              ...
              case count: Count =>
                ...
                val evaluatedExpr =
                  keyedResults
                    ...
                    .collect { case (key, resultSet -> value) =>
                      key -> (resultSet -> value)
                    }
                val countedPerKey = if (!count.isDistinct) {
                  evaluatedExpr
                    .mapValues { case (resultSet, _) =>
                      resultSet -> 1
                    }
                    .reduceByKey(reductionFunction)
                } else {
                  evaluatedExpr
                    .map { case (k, (resultSet, value)) =>
                ...
                countedPerKey.mapValues { case (resultSet, aggregatedCount) =>
                  val aggregatedResultSet = new MapBindingSet(1)
                  aggregatedResultSet.addBinding(
                    bindingName,
              ...
              case groupConcat: GroupConcat =>
                ...
                val evaluatedExpr =
                  keyedResults
                    .mapValues(resultSet =>
                      resultSet -> resultSet
                        .evaluateValueExpr(groupConcat.getArg)
                    )
                    .map { case (key, resultSet -> literal) =>
                      key -> (resultSet -> literal)
                    }
        ...
        case filter: Filter =>
          filter.getCondition match {
            case exists: Exists =>
              ...
              keyedLeft.join(keyedRight).values.collect { case (left, _) =>
                left
              }
