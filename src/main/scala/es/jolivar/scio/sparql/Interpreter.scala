...
object Interpreter {
  ...
  implicit class SCollectionStatements(val col: SCollection[Statement])
    ...
    /** Execute the query as is on the collection of triples provided by this
      * SCollection
      * @param query
      * @return
      *   the results of the SPARQL query with potential duplicates
      */
    def executeSparql(
        query: String
    ...
    /** Execute the query with the given bindings on the collection of triples
      * provided by this SCollection. The query used in practice will be
      * generated by replacing all instances of the bindings within the query.
      *
      * For example, if we have the query `SELECT ?a WHERE { ?a ?b ?c }` and the
      * binding `?c -> <iri>` then the query used will be `SELECT ?a WHERE { ?a
      * ?b <iri> }`.
      * @param query
      * @param bindings
      * @return
